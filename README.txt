...
-I meant to implement some gravity but didn't have much time (i.e. if a block from the bottom of the stack is picked up, the ones above it will fall down), so it is possible to have blocks that float in mid-air.

Enhancements:
-Blocks can stack on top of each other. If not dropped above another block, the block will drop to the ground.
-Some physics: showing path that a rotated block would follow when dropped. As the blocks are fragile, they are still returned to the original position.
